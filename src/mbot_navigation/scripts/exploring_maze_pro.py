...
######################################请补充更多位置（开始）######################################################
        xy = [[0.3,3.5],[0.3,6.5],[4,3.5],[4,6.5],[6,3.5],[6,6.5]]
        # for idx_x in range(5):
        #     for idx_y in range(5):
        #         xy.append([idx_x*2,idx_y*2+0.2])
        for idx in range(len(xy)):
            locations.append(Pose(Point(xy[idx][0], xy[idx][1], 0.000),  Quaternion(0,0,0,0)))  
        ...
        while not rospy.is_shutdown():
            ...
            if self.exploring_cmd is STATUS_EXPLORING:	# 探索，随机点导航
                if location < len(locations):
                    self.goal.target_pose.pose.position.x = locations[location].position.x
                    self.goal.target_pose.pose.position.y = locations[location].position.y
                    location += 1
                else:
