...
ros::Publisher geometryPub;


void pubGeometryMsg(const std::string& dataString)
{
...
int main(int argc, char* argv[])
{
	...
	geometryPub =  n.advertise<geometry_msgs::Twist>("cmd_vel", 1000);
	...
	const char* login_params = "appid = 9dd813f7, work_dir = .";
	...
	*/
	...
	while(ros::ok())
	{
		    ROS_INFO(" wakeupFlag = %d pid:%d",wakeupFlag,getpid());
			// 语音识别唤醒
			if(wakeupFlag ==1)
			{
				ROS_INFO("========== Wakeup...");
			...
			if(resultFlag){
					...
					std_msgs::String msg;
					msg.data = g_result;
					std::string stdStr(g_result);
					pubGeometryMsg(stdStr);
					ROS_INFO("%s",g_result);
